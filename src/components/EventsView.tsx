...
export default function EventsView({ events }: EventsViewProps) {
  ...
  // Submit News Tip Form State
  const [showTipForm, setShowTipForm] = useState(false);
  const [newTitle, setNewTitle] = useState('');
  const [newAuthor, setNewAuthor] = useState('');
  const [newCategory, setNewCategory] = useState<'Axé' | 'Samba' | 'Forró' | 'Jazz' | 'Alternativo'>('Axé');
  const [newNeighborhood, setNewNeighborhood] = useState('Rio Vermelho');
  const [newLocation, setNewLocation] = useState('');
  const [newSummary, setNewSummary] = useState('');
  const [newContent, setNewContent] = useState('');
  const [newImageUrl, setNewImageUrl] = useState(PRESET_IMAGES[2].url);
  ...
  // SUBMIT NEWS TIP FUNCTION (Creates raw news story in state feed)
  const handleSubmitNewsTip = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim() || !newAuthor.trim() || !newSummary.trim() || !newContent.trim()) {
      alert('Por favor, preencha todos os campos obrigatórios da pauta.');
      return;
    }

    const tipArticle: NewsArticle = {
      id: `tip-${Date.now()}`,
      title: `Boletim Comunitário: ${newTitle.trim()}`,
      date: 'Hoje mesmo',
      time: 'Publicado por Colaborador',
      location: newLocation.trim() || 'Espaço Público',
      neighborhood: newNeighborhood,
      category: newCategory,
      imageUrl: newImageUrl,
      badge: 'DICA DO LEITOR',
      ...
      comments: [
        {
          id: `tip-c-${Date.now()}`,
          author: 'Editor Sabor Salvador',
          content: 'Dica aprovada para exibição em nosso painel comunitário espontâneo. Obrigado por compartilhar seu axé!',
          timeAgo: 'Agora mesmo'
        }
      ]
    };

    setNewsList(prev => [tipArticle, ...prev]);
    ...
    // Toast Alert
    setToastMessage(`Sua notícia sobre "${newCategory}" em ${newNeighborhood} foi publicada com sucesso! 🎉`);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 text-left flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <button 
            ...
            className="bg-brand-primary text-white text-xs font-bold px-5 py-3 rounded-xl hover:bg-brand-primary-container transition shadow-md shadow-brand-primary/10 flex items-center gap-2 cursor-pointer self-start md:self-auto"
          >
            <Plus className="w-4 h-4" />
            <span>Enviar Dica de Evento (Pauta)</span>
          </button>
        </div>

        {/* SUBMIT EVENT NEWS TIP FORM (Aesthetic drawer widget) */}
        {showTipForm && (
          <div className="animate-fade-in mb-12 bg-white rounded-3xl p-6 md:p-8 border-2 border-brand-primary shadow-xl text-left">
            <div className="flex justify-between items-center border-b border-brand-container pb-4 mb-6">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-brand-primary" />
                <h2 className="font-display text-md font-bold text-brand-on-surface">Submeter Pauta / Cadastrar Notícia sobre Evento</h2>
              </div>
              <button 
                onClick={() => setShowTipForm(false)}
                className="text-brand-outline hover:text-brand-primary p-1 bg-brand-surface rounded-full"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <form onSubmit={handleSubmitNewsTip} className="space-y-4 text-xs font-semibold text-brand-on-surface-variant">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Manchete / Título Notícia *</label>
                  <input 
                    type="text" 
                    required
                    placeholder="Ex: Show de Jazz leva centenas ao Farol da Barra no fim de semana"
                    value={newTitle}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Seu Nome / Autor *</label>
                  <input 
                    type="text" 
                    required
                    placeholder="Ex: Jairo Antunes (Guia Local)"
                    value={newAuthor}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Ritmo Principal</label>
                  <select
                    value={newCategory}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Bairro de Salvador</label>
                  <select
                  ...
                  >
                    ...
                    <option value="Comércio">Comércio / Contorno</option>
                    <option value="Itapuã">Itapuã</option>
                  </select>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Endereço / Local (Opcional)</label>
                  <input 
                    type="text" 
                    placeholder="Ex: Varanda do Farol, Salvador"
                    value={newLocation}
              ...
              {/* Picture presets selection */}
              <div>
                <label className="block text-[10px] font-bold text-brand-outline uppercase mb-2">Tema Visual / Foto Ilustrativa</label>
                  ...
                  {PRESET_IMAGES.map((img) => (
                    <button
                      ...
                      className={`p-2 rounded-xl border text-center transition flex flex-col items-center gap-1.5 ${
                        newImageUrl === img.url
                          ? 'border-brand-primary bg-amber-50/10 shadow-xs'
                          : 'border-brand-outline-variant bg-brand-surface hover:bg-brand-container-low'
                      }`}
                    >
                      <div className="w-12 h-8 rounded overflow-hidden">
                        <img src={img.url} className="w-full h-full object-cover" />
                      </div>
                      <span className="text-[9px] font-extrabold block truncate w-full">{img.label}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Resumo Curto da Notícia *</label>
                <input 
                  type="text" 
                  required
                  placeholder="Ex: Uma tarde de harmonia e improvisação musical que tomou as escadarias coloniais do centro."
                  value={newSummary}
              ...
              <div>
                <label className="block text-[10px] font-bold text-brand-outline uppercase mb-1">Texto Completo da Pauta / Matéria *</label>
                <textarea 
                  rows={4}
                  required
                  value={newContent}
                  onChange={e => setNewContent(e.target.value)}
                  placeholder="Escreva em detalhes a reportagem, de preferência em português lírico explicando como transcorreu o acontecimento no bairro..." 
                  className="w-full bg-brand-surface border border-brand-outline-variant rounded-xl px-4 py-2.5 text-xs font-semibold focus:outline-none focus:border-brand-primary"
                />
              </div>

              <div className="pt-4 border-t border-brand-surface flex justify-end gap-3 text-xs">
                <button
                  type="button"
                  onClick={() => setShowTipForm(false)}
                  className="px-5 py-2.5 bg-brand-surface border border-brand-outline-variant text-brand-on-surface-variant font-bold rounded-xl hover:bg-brand-container transition"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-brand-primary hover:bg-brand-primary-container text-white font-bold rounded-xl shadow-md transition"
                >
                  Publicar Mural
                </button>
              </div>
